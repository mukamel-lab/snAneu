...
main_dir="/cndd/emukamel/Aneuploidy/ginkgo/scripts"
...
bm          = args[[6]] # binMeth - e.g. fixed_100000
cm          = args[[7]]
dm          = args[[8]]
...
normalization = as.numeric(args[[15]])
plotting    = args[[16]]
if (plotting==TRUE) { plotting="all" }
...
if (eran_process) {
    # ################ EAM modifications - for dealing with binc files not created from BAM files but from allc
    #
    # raw has columns "chr" and "bin_end". "bin_end" is the start of the bin.
    
    # Make sure that raw, loc, GC are all in the same order:
    names(raw)[1:2] <- c('CHR','START')
    raw['START'] = lapply(raw["START"],as.integer)
                      
    if (!grepl('chr',raw[1,'CHR'],fixed=TRUE)) {
     raw$CHR = paste('chr',raw$CHR,sep="")
    }
          
    df=loc
    df['GC'] = GC
    
    df['id'] = paste(df$CHR,df$START)
    raw['id'] = paste(raw$CHR,raw$START)
    
    df=merge(subset(df, select=-c(CHR,START)),raw, by='id')

    
    # Make sure chromosomes are sorted correctly
    df['CHRnum'] = df['CHR']
    df['CHRnum'] = lapply(df['CHRnum'],function (x) gsub('chr','',x))
    df['CHRnum'] = lapply(df['CHRnum'],function (x) gsub('X','100',x))
    df['CHRnum'] = lapply(df['CHRnum'],function (x) gsub('Y','200',x))
    df['CHRnum'] = lapply(df['CHRnum'],as.integer)
    
    df=df[order(df['CHRnum'],df['START']),]

    GC <- df['GC']
    loc <- df[c('CHR','START')]


    raw = subset(df, select=-c(CHR,START,GC,CHRnum,id))
    
    # Correct for the fact that we're counting CH sites, not reads
    raw = raw/normalization 
    
    # Recalculate bounds
    x=unique(loc['CHR'])
    bounds = data.frame(V1=as.list(x['CHR']),V2=c(1:dim(x)[1]))
    p=0
    for (chr in x[,'CHR']) {
        p=p+sum(loc['CHR']==chr)
        bounds[bounds['CHR']==chr,'V2'] = p
        }
    # ######################
}
...
colnames(stats) = c("Reads", "Bins", "Mean", "Std", "Disp", "Min", "25th", "Median", "75th", "Max") # The fourth column is the SD, not Variance
...
for(k in 1:w)
{
  ...
  # EAM: Bypass the automatic detection of global ploidy
  # print('*** Assuming globalploidy=2')
  # CN = 2
  ...
  if (plotting=="all") {
    ...
    jpeg(filename=paste("plots/", lab[k], "_dist.jpeg", sep=""), width=3000, height=750)
    ...
    jpeg(filename=paste("plots/", lab[k], "_counts.jpeg", sep=""), width=2500, height=1500)
    ...
    #Plot lorenz curves
    jpeg(filename=paste("plots/", lab[k], "_lorenz.jpeg", sep=""), width=2500, height=1500)
    ...
    #Plot GC correction
    jpeg(filename=paste("plots/", lab[k], "_GC.jpeg", sep=""), width=2500, height=1250)
    ...
    jpeg(filename=paste("plots/", lab[k], "_hist.jpeg", sep=""), width=2500, height=1500)
    ...
    jpeg(filename=paste("plots/", lab[k], "_SoS.jpeg", sep=""), width=2500, height=1500)
  ...
  if (plotting=="all" || plotting=="CN") {
    #Plot colored CN profile
    jpeg(filename=paste("plots/", lab[k], "_CN.jpeg", sep=""), width=3000, height=750)
...
# # ########### EAM: The heatmaps are not working currently; need to debug
